...
describe("get all favourites from a post by id endpoint test", () => {
  it("should get all post(id) favourites successfully", async () => {
    ...
    const res = await app.request(
      `/api/v1/post-favourites/post/${ONE_STEP_POST_FAVOURITE.postId}`,
      {
        method: "GET",
        headers: {
...
it("should not get all post(id) favourites if post does not exists", async () => {
  await registerUser(MANOLO_LOPEZ);
  const accessToken = await loginUser(MANOLO_LOPEZ);
  await insertPost(ONE_STEP_POST);
  await insertPostFavourite(ONE_STEP_POST_FAVOURITE);
  ...
  const res = await app.request(
    `/api/v1/post-favourites/post/${TWO_STEPS_POST.id}`,
    {
      method: "GET",
      headers: {
        Cookie: `accessToken=${accessToken}`,
