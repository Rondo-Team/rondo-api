...
describe("change post play by id endpoint tests", () => {
  it("should change a post play by id successfully", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
    await insertPost(TWO_STEPS_POST);

    const res = await app.request(`/api/v1/posts/${TWO_STEPS_POST.id}/play`, {
      method: "PATCH",
      headers: {
...
it("should not change a post play if it does not exists", async () => {
  await registerUser(MANOLO_LOPEZ);
  const accessToken = await loginUser(MANOLO_LOPEZ);
