...
describe("change draft information by id endpoint tests", () => {
  it("should cahnge a draft information by id successfully", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
    await insertDraft(TWO_STEPS_DRAFT);

    const res = await app.request(`/api/v1/drafts/${TWO_STEPS_DRAFT.id}/info`, {
      method: "PATCH",
      headers: {
      ...
      body: JSON.stringify({
        newTitle: ONE_STEP_DRAFT.title,
        newDescription: ONE_STEP_DRAFT.description,
      }),
    });
