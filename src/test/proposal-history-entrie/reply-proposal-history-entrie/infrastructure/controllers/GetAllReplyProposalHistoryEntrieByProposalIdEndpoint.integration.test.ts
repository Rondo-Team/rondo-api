...
import { insertReplyProposalHistoryEntrie } from "../../../../utils/insertReplyProposalHistoryEntrie.ts";
import {
  loginUser,
  registerUser,
...
describe("get all reply proposal history entries by proposal id endpoint tests", () => {
  it("should get all reply proposal history entries for a proposal by id successfully", async () => {
    ...
    await insertReplyProposalHistoryEntrie(REPLY_PROPOSAL_HISTORY_ENTRIE);

    const res = await app.request(
      `/api/v1/reply-proposal-history-entrie/proposal/${TWO_STEP_PROPOSAL.id}`,
      {
        method: "GET",
        headers: {
  ...
  it("should not get reply proposal history entries if proposal does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
