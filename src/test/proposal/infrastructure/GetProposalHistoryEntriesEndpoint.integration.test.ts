...
describe("get proposal history entries endpoint tests", () => {
  it("should get the proposal history entries successfully", async () => {
    ...
    const res = await app.request(
      `/api/v1/proposal/${TWO_STEP_PROPOSAL.id}/history`,
      {
        method: "GET",
        headers: {
  ...
  it("should return an empty array when the proposal does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
