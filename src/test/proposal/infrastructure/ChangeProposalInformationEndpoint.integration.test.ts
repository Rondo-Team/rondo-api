...
describe("change proposal information endpoint tests", () => {
  it("should change a proposal information successfully", async () => {
    ...
    const res = await app.request(
      `/api/v1/proposal/${TWO_STEP_PROPOSAL.id}/info`,
      {
        method: "PATCH",
        headers: {
  ...
  it("should not change a proposal information if it does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
  ...
  it("should not change a proposal information if user does not own it", async () => {
