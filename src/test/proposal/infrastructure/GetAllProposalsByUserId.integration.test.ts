...
describe("get all proposals by user id endpoint tests", () => {
  it("should get all proposals for a user by id successfully", async () => {
    ...
    const res = await app.request(`/api/v1/proposal/user/${MANOLO_LOPEZ.id}`, {
      method: "GET",
      headers: {
  ...
  it("should not get proposals if user does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
    ...
    const res = await app.request(
      `/api/v1/proposal/user/${nonExistentUserId}`,
      {
        method: "GET",
        headers: {
          Cookie: `accessToken=${accessToken}`,
