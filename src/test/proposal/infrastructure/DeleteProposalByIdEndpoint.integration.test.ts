...
describe("delete proposal by id endpoint tests", () => {
  it("should delete a proposal by id successfully", async () => {
    ...
    const res = await app.request(`/api/v1/proposal/${TWO_STEP_PROPOSAL.id}`, {
      method: "DELETE",
      headers: {
  ...
  it("should not delete a proposal by id if it does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
