...
describe("unmark comment as favourite endpoint tests", () => {
  it("should unmark a comment as favourite successfully", async () => {
    ...
    const res = await app.request(
      `/api/v1/comment-favourites/${SAMPLE_COMMENT_FAVOURITE.id}`,
      {
        method: "DELETE",
        headers: {
  ...
  it("should not unmark a comment favourite if it does not exist", async () => {
    await registerUser(MANOLO_LOPEZ);
    const accessToken = await loginUser(MANOLO_LOPEZ);
