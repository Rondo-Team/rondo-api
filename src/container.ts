...
import { UpdateProposalEndpoint } from "./proposal/infrastructure/controllers/UpdateProposalEndpoint.ts";
...
container
  .bind(Token.GET_ALL_COMMENT_FAVOURITES_BY_USER_ID_AND_POST_ID)
  .toDynamicValue(async (ctx) => {
    return new GetAllCommentFavouritesByUserIdAndPostId(
      await ctx.getAsync<CommentFavouriteRepository>(
        Token.COMMENT_FAVOURITE_REPOSITORY,
      ),
      await ctx.getAsync<UserRepository>(Token.USER_REPOSITORY),
...
container
  .bind(Token.REPLY_PROPOSAL)
  .toDynamicValue(async (ctx) => {
    return new ReplyProposal(
      await ctx.getAsync<ProposalRepository>(Token.PROPOSAL_REPOSITORY),
      await ctx.getAsync<PostRepository>(Token.POST_REPOSITORY),
    );
  })
...
container
  .bind(Token.ENDPOINT)
  .toDynamicValue(async (ctx) => {
    const replyProposal = await ctx.getAsync<ReplyProposal>(
...
container
  .bind(Token.ENDPOINT)
  .toDynamicValue(async (ctx) => {
    const updateProposal = await ctx.getAsync<UpdateProposal>(
      Token.UPDATE_PROPOSAL,
    );
    return UpdateProposalEndpoint(updateProposal);
  })
  .inSingletonScope();
