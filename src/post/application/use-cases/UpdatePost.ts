...
export class UpdatePost {
  ...
  async run(
    ...
    if (newPlay) post.changePlay(Play.fromPrimitives(newPlay));
    if (newTitle) post.changeTitle(PostTitle.fromPrimitives(newTitle));
    if (newDescription)
      post.changeDescription(PostDescription.fromPrimitives(newDescription));
    if (newTags) post.changeTags(PostTags.fromPrimitives(newTags));
